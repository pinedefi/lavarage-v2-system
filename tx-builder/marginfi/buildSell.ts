...
async function main() {
    ...
    const { offer, tradeAmount, slippage, userPubKey } = JSON.parse(process.argv[2]) as { offer: MarginOffer, tradeAmount: number, slippage: number, userPubKey: string };
    ...
    const jupSwapIxEncoded = await getSwapIx({
        from: offer.collateral_token,
        to: offer.borrow_token,
        amount: tradeAmount,
        slippage,
    ...
    const borrowIx = await marginfiAccount.makeBorrowIx(tradeAmount/1000000, collateralBank.address);
    const balance = marginfiAccount.getBalance(solBank.address);
    const toRepay = balance.computeQuantityUi(solBank)
    const depositIx = await marginfiAccount.makeRepayIx(toRepay.liabilities, solBank.address);
